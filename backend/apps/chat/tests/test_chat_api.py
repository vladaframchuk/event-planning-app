...
def test_participant_can_list_and_post_messages_in_event() -> None:
    ...
    post_response = client.post(
        f"/api/events/{event.id}/messages", {"text": "   Новый текст   "}, format="json"
    )
...
def test_non_participant_forbidden() -> None:
    ...
    post_response = client.post(
        f"/api/events/{event.id}/messages", {"text": "Привет"}, format="json"
    )
...
def test_messages_order_is_ascending_by_created_at() -> None:
    ...
    Message.objects.filter(id=middle.id).update(
        created_at=older.created_at - timedelta(seconds=10)
    )
    Message.objects.filter(id=newer.id).update(
        created_at=older.created_at + timedelta(seconds=5)
    )
...
def test_pagination_and_before_after_id_filters() -> None:
    event, owner = _create_event()
    participant = User.objects.create_user(
        email="pages@example.com", password="Password123"
    )
    Participant.objects.create(
        event=event, user=participant, role=Participant.Role.MEMBER
    )
    ...
    for index in range(1, 36):
        message = Message.objects.create(
            event=event, author=owner, text=f"Сообщение {index}"
        )
    ...
    assert [item["text"] for item in first_page.json()["results"]] == [
        f"Сообщение {i}" for i in range(1, 6)
    ]
    ...
    assert [item["text"] for item in before_page.json()["results"]] == [
        f"Сообщение {i}" for i in range(6, 11)
    ]
...
def test_rate_limit_simple_antispam_429() -> None:
    event, _ = _create_event()
    participant = User.objects.create_user(
        email="spam@example.com", password="Password123"
    )
    Participant.objects.create(
        event=event, user=participant, role=Participant.Role.MEMBER
    )
    ...
    client = _auth_client(participant)
    first = client.post(
        f"/api/events/{event.id}/messages", {"text": "Первое"}, format="json"
    )
    assert first.status_code == 201

    second = client.post(
        f"/api/events/{event.id}/messages", {"text": "Второе"}, format="json"
    )
...
def test_trim_and_reject_empty_or_long_text() -> None:
    ...
    response = client.post(
        f"/api/events/{event.id}/messages", {"text": "   Обрезка   "}, format="json"
    )
    assert response.status_code == 201
    assert response.json()["text"] == "Обрезка"

    empty_response = client.post(
        f"/api/events/{event.id}/messages", {"text": "   "}, format="json"
    )
    assert empty_response.status_code == 400

    long_text = "x" * 4001
    too_long_response = client.post(
        f"/api/events/{event.id}/messages", {"text": long_text}, format="json"
    )
