...
const containerStyle = {
  paddingTop: 'calc(env(safe-area-inset-top) + var(--space-2xl))',
  paddingBottom: 'calc(env(safe-area-inset-bottom) + var(--space-xl))',
  minHeight: 'min(calc(100svh - 72px), 760px)',
} as const;
...
const Hero = (): JSX.Element => {
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-6xl flex-col items-start gap-12 px-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        <div className="flex w-full max-w-2xl flex-col gap-6 text-left">
          ...
          <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center">
            <Link href="/events" className={primaryButtonClasses}>
              {t('landing.hero.primaryCta')}
            </Link>
            <Link href="/events" className={secondaryButtonClasses}>
